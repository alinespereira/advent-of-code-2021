...
object Day03 extends App with PuzzleResource {
  ...
  def lifeSupport(data: List[String]): Int = {
    ...
    @tailrec
    def filterData(bitFilter: (Int, Int) => Int)(data: List[String], index: Int = 0): String = {
      if (data.length == 1) data.head
      else {
        val counter = reportCounter(data)
        val bit = bitFilter(counter(index)._1, counter(index)._2)
        filterData(bitFilter)(data.filter(p => p(index).asDigit == bit), index + 1)
      }
    }
    ...
    val oxygenRating = toDecimal(filterData(oxygenFilter)(data).toList.map(_.asDigit))
    val scrubberRating = toDecimal(filterData(scrubberFilter)(data).toList.map(_.asDigit))
